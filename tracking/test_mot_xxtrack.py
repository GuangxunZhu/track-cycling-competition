...
import tracker.xxtracker.xxtracker as tracker_builder
...
def run_seq(opt, dataloader, result_filename, seq):
    ...
    if opt.cfg.MODEL.Name in ['JDE', 'CSTrack', 'OMC']:
        tracker = tracker_builder.BYTETracker(opt, frame_rate=opt.frame_rate)
    else:
    ...
    model = model_builder.build(pre_save_cfg=None).to(opt.device) 
    ...
    for path, img, img0, frame_id_start in dataloader:
        ...
        with torch.no_grad():
            output = model(blob, augment = False)
            pred, train_out = output[1]
            id_feature = output[0]
        detections = []
        if len(pred) > 0:
            dets,x_inds,y_inds = det_helper.non_max_suppression_and_inds(pred[:,:,:6].unsqueeze(0), opt.conf_thres, opt.nms_thres,method='cluster_diou')
            det_helper.scale_coords(opt.img_size, dets[:, :4], img0.shape).round()
            detections = dets.cpu().numpy()



        online_targets = tracker.update(detections, id_feature, img0)   # no init process for MOT
