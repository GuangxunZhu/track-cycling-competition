...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='',is_json=True):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',is_json=True):
        self.img_size = img_size
        ...
        try:
            ...
            for ds, path in path.items():  # ds: dataset name, e.g. mot15
                with open(path, 'r') as file:
                    self.img_files_dict[ds] = file.readlines()
                    self.img_files_dict[ds] = [x.strip() for x in self.img_files_dict[ds]]
                    self.img_files_dict[ds] = list(filter(lambda x: len(x) > 0, self.img_files_dict[ds]))

                self.label_files_dict[ds] = img2label_paths(self.img_files_dict[ds])
            ...
            for ds, label_paths in self.label_files_dict.items():
                ...
                logger.info('dataset {0}, max target id: {1} (start from 1)'.format(ds, max_index))
            ...
            for i, (k, v) in enumerate(self.tid_num.items()):
                ...
                last_index += v
                logger.info('dataset {0}, start id: {1}, end id: {2}'.format(k, self.tid_start_index[k], last_index))
        ...
        cache_path =(path.split('/')[0]+'/' if Path(path).is_file() else self.label_files[0].split('/')[:-1]+'/')
        for key in self.img_files_dict.keys():
            cache_path+=key+'_'

        cache_path=Path(cache_path.strip('_')).with_suffix('.cache') # cached labels
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        ...
                        if any([len(x) > 8 for x in l]):  # is segment
                            ...
                            segments = [np.array(x[2:], dtype=np.float32).reshape(-1, 2) for x in l]  # (cls, xy1...)
                            l = np.concatenate((classes.reshape(-1, 1), segments2boxes(segments)), 1)  # (cls, xywh)
                        l = np.array(l, dtype=np.float32)
                    if len(l):
                        assert l.shape[1] == 6, 'labels require 5 columns each'
                        # assert (l[:,2:] >= 0).all(), 'negative labels'
...
def copy_paste(img, labels, segments, probability=0.5):
    ...
    if probability and n:
        ...
        for j in random.sample(range(n), k=round(probability * n)):
            ...
            box = w - l[4], l[3], w - l[2], l[5]
            ioa = bbox_ioa(box, labels[:, 2:6])  # intersection over area
