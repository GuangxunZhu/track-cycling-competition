...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, nID, emb_dim,autobalance=False ):
    ...
    def __call__(self, output, targets):  # predictions, targets, model
        if isinstance(output[1][0],list):
            p = output[1][0]
            id_embeding =output[0]
        else:
            p=[x.float() for x in output]
            id_embeding=torch.tensor([])
        device = targets.device
        
        lcls, lbox, lobj, id_loss = torch.zeros(1, device=device), torch.zeros(1, device=device),\
                                    torch.zeros(1, device=device), torch.zeros(1, device=device)
        tcls, tbox, indices, anchors, indices_id, tids = self.build_targets(p, targets)  # targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                #idloss
                # if i == 0 and id_embeding is not None:
    ...
    def build_targets(self, p, targets):
        ...
        indices_id, tids = [], []
        targets=torch.cat((targets[:,:1],targets[:,2:]),dim=1)
        ...
        for i in range(self.nl):
            ...
            # gxy_id = t[0][:, 2:4]  # grid xy
            # gwh_id = t[0][:, 4:6]  # grid wh
            # gij_id = gxy_id.long()
            # gi_id, gj_id = gij_id.T  # grid xy indices
            # # Append
            # indices_id.append((b, gj_id, gi_id))  # image, anchor, grid indices
            # tids.append(c)  # class
...
class ComputeLossOTA:
    ...
    def __call__(self, output, targets, imgs):  # predictions, targets, model   
        device = targets.device
        ...
        lcls, lbox, lobj, id_loss = torch.zeros(1, device=device), torch.zeros(1, device=device),\
                                    torch.zeros(1, device=device), torch.zeros(1, device=device)
        bs, as_, gjs, gis, targets, anchors, indices_id, tids = self.build_targets(p, targets, imgs)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
                pbox = torch.cat((pxy, pwh), 1)  # predicted box
                selected_tbox = targets[i][:, 3:7] * pre_gen_gains[i]
        ...
        loss = lbox + lobj + lcls+ id_loss
        return loss * bs, torch.cat((lbox, lobj, id_loss, loss)).detach()

    def build_targets(self, p, targets, imgs):
        
        #indices, anch = self.find_positive(p, targets)
        indices, anch, indices_id, tids = self.find_3_positive(p, targets)
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            txywh = this_target[:, 3:7] * imgs[batch_idx].shape[1]
        ...
        return matching_bs, matching_as, matching_gjs, matching_gis, matching_targets, matching_anchs ,indices_id, tids           

    def find_3_positive(self, p, targets):
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
        na, nt = self.na, targets.shape[0]  # number of anchors, targets
        indices, anch = [], []
        targets=torch.cat((targets[:,0:1],targets[:,2:]),dim=1) 
        indices_id, tids = [], []
        ...
        for i in range(self.nl):
            ...
            b , c = t[0][:, 0:2].long().T  # image, class
            occ = t[0][:,-2].T
            mask = (c != 0 ) & (occ != 1)
            c=c[mask]
            b=b[mask]
            ...
            gxy_id = t[0][:, 2:4][mask]  # grid xy
            gwh_id = t[0][:, 4:6][mask]  # grid wh
            gij_id = gxy_id.long()
            gi_id, gj_id = gij_id.T  # grid xy indices
            # Append
            indices_id.append((b, gj_id, gi_id))  # image, anchor, grid indices
        ...
        return indices, anch, indices_id, tids
