...
def check_dataset(dict):
    ...
    if val and len(val):
        for v in val.values():
            val = [Path(x).resolve() for x in (v if isinstance(v, list) else [v])]  # val path
...
def get_id_inds(boxes):
    if len(boxes) != 0:
        x_inds = (boxes[:, 0] + boxes[:, 2]) // 16
        y_inds = (boxes[:, 1] + boxes[:, 3]) // 16
        x_inds[x_inds >= 48] = 47
        # y_inds[y_inds < 0] = 0
        y_inds[y_inds >= 80] = 79
        # x_inds[x_inds < 0] = 0
...
# def non_max_suppression_and_inds(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
#                         labels=()):
#     """Runs Non-Maximum Suppression (NMS) on inference results
