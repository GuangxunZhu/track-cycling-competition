...
class Bicycle(ImageDataset):
    ...
    def __init__(self, root='datasets', **kwargs):
        ...
        data_dir = osp.join(self.data_dir, 'Bicycle-ReID')
        ...
        super(Bicycle, self).__init__(train, query, gallery, **kwargs)

    def process_dir(self, dir_path, is_train=True):

        img_paths = glob.glob(osp.join(dir_path, '*.bmp'))
        pattern = re.compile(r'([-\d]+)_Bicycle')
